...
#SBATCH --output=/home/jefcheng/dev/martiandating/results/slurm_N4_%j.log

cd /home/jefcheng/dev/martiandating
export JAX_COMPILATION_CACHE_DIR=/home/jefcheng/.cache/jax
echo "Starting N4 on $(hostname) at $(date)"
...
for gpu_id in 0 1 2 3 4 5 6 7; do
    CUDA_VISIBLE_DEVICES=$gpu_id /apcv/shared/conda-envs/ai-7924-cuda-x86/bin/python run_gpu.py \
        ...
        --tag N4_gpu${gpu_id} --seed $((42 + gpu_id)) \
        --num_domains 4 --ea_mode fixed --learn_sigma --noise_penalty_rate 5.0 --dirichlet_alpha 2.0 \
        > /home/jefcheng/dev/martiandating/results/N4_gpu${gpu_id}.log 2>&1 &
done
wait
...
for g in range(8):
    try:
        d = pickle.load(open('/home/jefcheng/dev/martiandating/results/fit_N4_gpu' + str(g) + '.pkl','rb'))
        for k in d['samples']:
...
if all_s:
    ...
    pickle.dump({'samples': combined, 'tag': 'N4'}, open('/home/jefcheng/dev/martiandating/results/fit_N4_combined.pkl', 'wb'))
    print(f'N4: {combined[list(combined.keys())[0]].shape}')
    if 'phi' in combined:
...
/apcv/shared/conda-envs/ai-7924-cuda-x86/bin/python plot_final.py 2>&1 | grep "N4"
