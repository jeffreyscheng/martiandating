...
#SBATCH --output=/home/jefcheng/dev/martiandating/results/slurm_D4_%j.log
...
echo "Starting D4 on $(hostname) at $(date)"
...
for gpu_id in 0 1 2 3 4 5 6 7; do
    CUDA_VISIBLE_DEVICES=$gpu_id $PY run_gpu.py \
        ...
        --tag D4_gpu${gpu_id} --seed $((42 + gpu_id)) \
        --num_domains 4 --ea_mode fixed \
        > /home/jefcheng/dev/martiandating/results/D4_gpu${gpu_id}.log 2>&1 &
done
wait
...
for g in range(8):
    try:
        d = pickle.load(open('/home/jefcheng/dev/martiandating/results/fit_D4_gpu' + str(g) + '.pkl','rb'))
        for k in d['samples']:
...
if all_s:
    ...
    pickle.dump({'samples': combined, 'tag': 'D4'}, open('/home/jefcheng/dev/martiandating/results/fit_D4_combined.pkl', 'wb'))
    print(f'D4: {combined[list(combined.keys())[0]].shape}')
"
