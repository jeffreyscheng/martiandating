...
## Scope and primary duration result

This target analyzes Nakhla, whose closure age is about 1.3 Ga. It does not by
itself test the older Noachian denudation record; the four-billion-year scope
of Shuster and Weiss also used ALH84001.

The primary temperature prior is uniform from -100 to 100 C. At temperatures
cold enough to produce negligible diffusion, the likelihood is flat and the
lower posterior tail remains prior-dominated. Temperature medians must not be
presented as estimates of the actual Martian climate. The upper tail is the
relevant conditional constraint, and prior-bound sensitivity is required.

Two independent runs were completed for excursions ending at the present,
with 32 chains, 2,000 warmup steps, and 2,000 retained draws per run. Pooled
results contain 128,000 states per duration:

| duration | central 95% and median (C) | P(T > 0 C) | temperature R-hat | bulk / tail ESS |
| --- | --- | --- | --- | --- |
| 10 My | [-94.13, -7.73, 7.33] | 0.2138 | 1.0029 | 19,177 / 36,246 |
| 100 My | [-94.63, -18.58, -4.47] | 0.0026 | 1.0032 | 18,112 / 34,541 |
| 200 My | [-94.73, -21.62, -7.67] | 0.0003 | 1.0027 | 18,241 / 34,171 |
| 500 My | [-94.93, -25.27, -11.50] | 0.00001 | 1.0031 | 18,163 / 34,601 |
| 1,300 My | [-95.08, -27.52, -13.84] | 0 | 1.0024 | 19,135 / 33,815 |

All ten runs had zero divergences and zero maximum-depth transitions. These
numbers condition on no natural diffusion outside the one specified excursion.
They are not a reconstruction of a continuous background temperature history.
Figure 3 varies the start time of each excursion, so the present-ending rows
above are only one edge of the required timing surface.

## Expanded-ensemble timing grid

The timing surface is sampled in one expanded ensemble rather than by
refitting the same laboratory experiment once per grid cell. Let `s` index a
mutually exclusive duration/onset scenario and let `x = (theta, T)` contain
the continuous parameters. With declared grid weights `pi_s`, NUTS targets

```text
p_mix(x | y) proportional to p(x) sum_s pi_s p(y | x, s).
```

The observed spectrum occurs once inside this sum. A product over scenarios
would count the same data repeatedly and incorrectly assert that all mutually
exclusive excursions happened. For each retained draw, the responsibility

```text
r_s(x) = pi_s p(y | x, s) / sum_k pi_k p(y | x, k)
```

recovers conditional expectations by weighted averaging:

```text
E[f(x) | y, s] = E_mix[f(x) r_s(x)] / E_mix[r_s(x)].
```

Thus each grid-conditioned posterior is the same posterior targeted by an
independent fit, while the shared laboratory and diffusion calculations are
performed once. The old independent runs remain useful only as numerical
regression checks. Expanded-ensemble validation additionally requires at
least 1,000 responsibility-weighted effective samples per cell and agreement
of mean cell responsibility across chains to within 25% relative deviation.
